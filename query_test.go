...
func TestFirstAndLast(t *testing.T) {
	DB.Save(context.Background(), &User{Name: "user1", Emails: []Email{{Email: "user1@example.com"}}})
	DB.Save(context.Background(), &User{Name: "user2", Emails: []Email{{Email: "user2@example.com"}}})

	var user1, user2, user3, user4 User
	DB.First(context.Background(), &user1)
	DB.Order("id").Limit(1).Find(context.Background(), &user2)

	ptrOfUser3 := &user3
	DB.Last(context.Background(), &ptrOfUser3)
	DB.Order("id desc").Limit(1).Find(context.Background(), &user4)
	if user1.Id != user2.Id || user3.Id != user4.Id {
		t.Errorf("First and Last should by order by primary key")
	}

	var users []User
	DB.First(context.Background(), &users)
	if len(users) != 1 {
		t.Errorf("Find first record as slice")
	}

	var user User
	if DB.Joins("left join emails on emails.user_id = users.id").First(context.Background(), &user).Error != nil {
...
func TestFirstAndLastWithNoStdPrimaryKey(t *testing.T) {
	DB.Save(context.Background(), &Animal{Name: "animal1"})
	DB.Save(context.Background(), &Animal{Name: "animal2"})

	var animal1, animal2, animal3, animal4 Animal
	DB.First(context.Background(), &animal1)
	DB.Order("counter").Limit(1).Find(context.Background(), &animal2)

	DB.Last(context.Background(), &animal3)
	DB.Order("counter desc").Limit(1).Find(context.Background(), &animal4)
...
func TestFirstAndLastWithRaw(t *testing.T) {
	...
	DB.Raw("select * from users WHERE name = ?", "user").First(context.Background(), &user3)
	if user3.Id != user1.Id {
		t.Errorf("Find first record with raw")
	}

	DB.Raw("select * from users WHERE name = ?", "user").Last(context.Background(), &user4)
...
func TestUIntPrimaryKey(t *testing.T) {
	var animal Animal
	DB.First(context.Background(), &animal, uint64(1))
	if animal.Counter != 1 {
		t.Errorf("Fetch a record from with a non-int primary key should work, but failed")
	}

	DB.Model(Animal{}).Where(Animal{Counter: uint64(2)}).Scan(context.Background(), &animal)
	if animal.Counter != 2 {
		t.Errorf("Fetch a record from with a non-int primary key should work, but failed")
	}
}
...
func TestCustomizedTypePrimaryKey(t *testing.T) {
	...
	DB.Create(context.Background(), &p1)
	DB.Create(context.Background(), &p2)
	DB.Create(context.Background(), &p3)

	var p CustomizedTypePrimaryKey

	if err := DB.First(context.Background(), &p, p2.ID).Error; err == nil {
		t.Errorf("Should return error for invalid query condition")
	}

	if err := DB.First(context.Background(), &p, "id = ?", p2.ID).Error; err != nil {
...
func TestStringPrimaryKeyForNumericValueStartingWithZero(t *testing.T) {
	...
	DB.Create(context.Background(), &AddressByZipCode{ZipCode: "00501", Address: "Holtsville"})

	var address AddressByZipCode
	DB.First(context.Background(), &address, "00501")
...
func TestFindAsSliceOfPointers(t *testing.T) {
	DB.Save(context.Background(), &User{Name: "user"})

	var users []User
	DB.Find(context.Background(), &users)

	var userPointers []*User
	DB.Find(context.Background(), &userPointers)
...
func TestSearchWithPlainSQL(t *testing.T) {
	...
	if DB.Where("name LIKE ?", "%"+user1.Name+"%").First(context.Background(), &User{}).RecordNotFound() {
		t.Errorf("Search with plan SQL (regexp)")
	}

	var users []User
	DB.Find(context.Background(), &users, "name LIKE ? and age > ?", "%PlainSqlUser%", 1)
	if len(users) != 2 {
		t.Errorf("Should found 2 users that age > 1, but got %v", len(users))
	}

	DB.Where("name LIKE ?", "%PlainSqlUser%").Where("age >= ?", 1).Find(context.Background(), &users)
	if len(users) != 3 {
		t.Errorf("Should found 3 users that age >= 1, but got %v", len(users))
	}

	scopedb.Where("age <> ?", 20).Find(context.Background(), &users)
	if len(users) != 2 {
		t.Errorf("Should found 2 users age != 20, but got %v", len(users))
	}

	scopedb.Where("birthday > ?", parseTime("2000-1-1")).Find(context.Background(), &users)
	if len(users) != 2 {
		t.Errorf("Should found 2 users's birthday > 2000-1-1, but got %v", len(users))
	}

	scopedb.Where("birthday > ?", "2002-10-10").Find(context.Background(), &users)
	if len(users) != 2 {
		t.Errorf("Should found 2 users's birthday >= 2002-10-10, but got %v", len(users))
	}

	scopedb.Where("birthday >= ?", "2010-1-1").Where("birthday < ?", "2020-1-1").Find(context.Background(), &users)
	if len(users) != 1 {
		t.Errorf("Should found 1 users's birthday < 2020-1-1 and >= 2010-1-1, but got %v", len(users))
	}

	DB.Where("name in (?)", []string{user1.Name, user2.Name}).Find(context.Background(), &users)
	if len(users) != 2 {
		t.Errorf("Should found 2 users, but got %v", len(users))
	}

	DB.Where("id in (?)", []int64{user1.Id, user2.Id, user3.Id}).Find(context.Background(), &users)
	if len(users) != 3 {
		t.Errorf("Should found 3 users, but got %v", len(users))
	}

	DB.Where("id in (?)", user1.Id).Find(context.Background(), &users)
	if len(users) != 1 {
		t.Errorf("Should found 1 users, but got %v", len(users))
	}

	if err := DB.Where("id IN (?)", []string{}).Find(context.Background(), &users).Error; err != nil {
		t.Error("no error should happen when query with empty slice, but got: ", err)
	}

	if err := DB.Not("id IN (?)", []string{}).Find(context.Background(), &users).Error; err != nil {
		t.Error("no error should happen when query with empty slice, but got: ", err)
	}

	if DB.Where("name = ?", "none existing").Find(context.Background(), &[]User{}).RecordNotFound() {
...
func TestSearchWithTwoDimensionalArray(t *testing.T) {
	...
	if dialect := DB.Dialect().GetName(); dialect == "mysql" || dialect == "postgres" {
		if err := DB.Where("(name, age) IN (?)", [][]interface{}{{"2DSearchUser1", 1}, {"2DSearchUser2", 10}}).Find(context.Background(), &users).Error; err != nil {
	...
	if dialect := DB.Dialect().GetName(); dialect == "mssql" {
		if err := DB.Joins("JOIN (VALUES ?) AS x (col1, col2) ON x.col1 = name AND x.col2 = age", [][]interface{}{{"2DSearchUser1", 1}, {"2DSearchUser2", 10}}).Find(context.Background(), &users).Error; err != nil {
			t.Errorf("No error should happen when query with 2D array, but got %v", err)
...
func TestSearchWithStruct(t *testing.T) {
	...
	if DB.Where(user1.Id).First(context.Background(), &User{}).RecordNotFound() {
		t.Errorf("Search with primary key")
	}

	if DB.First(context.Background(), &User{}, user1.Id).RecordNotFound() {
		t.Errorf("Search with primary key as inline condition")
	}

	if DB.First(context.Background(), &User{}, fmt.Sprintf("%v", user1.Id)).RecordNotFound() {
		t.Errorf("Search with primary key as inline condition")
	}

	var users []User
	DB.Where([]int64{user1.Id, user2.Id, user3.Id}).Find(context.Background(), &users)
	if len(users) != 3 {
		t.Errorf("Should found 3 users when search with primary keys, but got %v", len(users))
	}

	var user User
	DB.First(context.Background(), &user, &User{Name: user1.Name})
	...
	DB.Where(&User{Name: user1.Name}).First(context.Background(), &user)
	if user.Id == 0 || user.Name != user1.Name {
		t.Errorf("Search first record with where struct")
	}

	DB.Find(context.Background(), &users, &User{Name: user2.Name})
	if len(users) != 1 {
...
func TestSearchWithMap(t *testing.T) {
	...
	DB.Save(context.Background(), &user1).Save(context.Background(), &user2).Save(context.Background(), &user3).Save(context.Background(), &user4)

	var user User
	DB.First(context.Background(), &user, map[string]interface{}{"name": user1.Name})
	if user.Id == 0 || user.Name != user1.Name {
		t.Errorf("Search first record with inline map")
	}

	user = User{}
	DB.Where(map[string]interface{}{"name": user2.Name}).First(context.Background(), &user)
	if user.Id == 0 || user.Name != user2.Name {
		t.Errorf("Search first record with where map")
	}

	var users []User
	DB.Where(map[string]interface{}{"name": user3.Name}).Find(context.Background(), &users)
	if len(users) != 1 {
		t.Errorf("Search all records with inline map")
	}

	DB.Find(context.Background(), &users, map[string]interface{}{"name": user3.Name})
	if len(users) != 1 {
		t.Errorf("Search all records with inline map")
	}

	DB.Find(context.Background(), &users, map[string]interface{}{"name": user4.Name, "company_id": nil})
	if len(users) != 0 {
		t.Errorf("Search all records with inline map containing null value finding 0 records")
	}

	DB.Find(context.Background(), &users, map[string]interface{}{"name": user1.Name, "company_id": nil})
	if len(users) != 1 {
		t.Errorf("Search all records with inline map containing null value finding 1 record")
	}

	DB.Find(context.Background(), &users, map[string]interface{}{"name": user4.Name, "company_id": companyID})
	if len(users) != 1 {
...
func TestSearchWithEmptyChain(t *testing.T) {
	...
	if DB.Where("").Where("").First(context.Background(), &User{}).Error != nil {
		t.Errorf("Should not raise any error if searching with empty strings")
	}

	if DB.Where(&User{}).Where("name = ?", user1.Name).First(context.Background(), &User{}).Error != nil {
		t.Errorf("Should not raise any error if searching with empty struct")
	}

	if DB.Where(map[string]interface{}{}).Where("name = ?", user1.Name).First(context.Background(), &User{}).Error != nil {
...
func TestSelect(t *testing.T) {
	...
	var user User
	DB.Where("name = ?", user1.Name).Select("name").Find(context.Background(), &user)
	if user.Id != 0 {
...
func TestOrderAndPluck(t *testing.T) {
	...
	var user User
	scopedb.Order(gorm.Expr("case when name = ? then 0 else 1 end", "OrderPluckUser2")).First(context.Background(), &user)
	if user.Name != "OrderPluckUser2" {
		t.Errorf("Order with sql expression")
	}

	var ages []int64
	scopedb.Order("age desc").Pluck(context.Background(), "age", &ages)
	if ages[0] != 20 {
		t.Errorf("The first age should be 20 when order with age desc")
	}

	var ages1, ages2 []int64
	scopedb.Order("age desc").Pluck(context.Background(), "age", &ages1).Pluck(context.Background(), "age", &ages2)
	if !reflect.DeepEqual(ages1, ages2) {
		t.Errorf("The first order is the primary order")
	}

	var ages3, ages4 []int64
	scopedb.Model(&User{}).Order("age desc").Pluck(context.Background(), "age", &ages3).Order("age", true).Pluck(context.Background(), "age", &ages4)
	...
	var names []string
	var ages5 []int64
	scopedb.Model(User{}).Order("name").Order("age desc").Pluck(context.Background(), "age", &ages5).Pluck(context.Background(), "name", &names)
	...
	var ages6 []int64
	if err := scopedb.Order("").Pluck(context.Background(), "age", &ages6).Error; err != nil {
		t.Errorf("An empty string as order clause produces invalid queries")
	}

	DB.Model(User{}).Select("name, age").Find(context.Background(), &[]User{})
}
...
func TestLimit(t *testing.T) {
	...
	DB.Save(context.Background(), &user1).Save(context.Background(), &user2).Save(context.Background(), &user3).Save(context.Background(), &user4).Save(context.Background(), &user5)

	var users1, users2, users3 []User
	DB.Order("age desc").Limit(3).Find(context.Background(), &users1).Limit(5).Find(context.Background(), &users2).Limit(-1).Find(context.Background(), &users3)
...
func TestOffset(t *testing.T) {
	for i := 0; i < 20; i++ {
		DB.Save(context.Background(), &User{Name: fmt.Sprintf("OffsetUser%v", i)})
	}
	var users1, users2, users3, users4 []User
	DB.Limit(100).Where("name like ?", "OffsetUser%").Order("age desc").Find(context.Background(), &users1).Offset(3).Find(context.Background(), &users2).Offset(5).Find(context.Background(), &users3).Offset(-1).Find(context.Background(), &users4)
...
func TestOr(t *testing.T) {
	...
	var users []User
	DB.Where("name = ?", user1.Name).Or("name = ?", user2.Name).Find(context.Background(), &users)
	if len(users) != 2 {
...
func TestCount(t *testing.T) {
	...
	if err := DB.Where("name = ?", user1.Name).Or("name = ?", user3.Name).Find(context.Background(), &users).Count(context.Background(), &count).Error; err != nil {
	...
	DB.Model(&User{}).Where("name = ?", user1.Name).Count(context.Background(), &count1).Or("name in (?)", []string{user2.Name, user3.Name}).Count(context.Background(), &count2)
	if count1 != 1 || count2 != 3 {
		t.Errorf("Multiple count in chain")
	}

	var count3 int
	if err := DB.Model(&User{}).Where("name in (?)", []string{user2.Name, user2.Name, user3.Name}).Group("id").Count(context.Background(), &count3).Error; err != nil {
...
func TestNot(t *testing.T) {
	DB.Create(context.Background(), getPreparedUser("user1", "not"))
	DB.Create(context.Background(), getPreparedUser("user2", "not"))
	DB.Create(context.Background(), getPreparedUser("user3", "not"))

	user4 := getPreparedUser("user4", "not")
	user4.Company = Company{}
	DB.Create(context.Background(), user4)

	DB := DB.Where("role = ?", "not")

	var users1, users2, users3, users4, users5, users6, users7, users8, users9 []User
	if DB.Find(context.Background(), &users1).RowsAffected != 4 {
		t.Errorf("should find 4 not users")
	}
	DB.Not(users1[0].Id).Find(context.Background(), &users2)

	if len(users1)-len(users2) != 1 {
		t.Errorf("Should ignore the first users with Not")
	}

	DB.Not([]int{}).Find(context.Background(), &users3)
	if len(users1)-len(users3) != 0 {
		t.Errorf("Should find all users with a blank condition")
	}

	var name3Count int64
	DB.Table("users").Where("name = ?", "user3").Count(context.Background(), &name3Count)
	DB.Not("name", "user3").Find(context.Background(), &users4)
	...
	DB.Not(User{Name: "user3"}).Find(context.Background(), &users5)

	if len(users1)-len(users5) != int(name3Count) {
		t.Errorf("Should find all users's name not equal 3")
	}

	DB.Not(map[string]interface{}{"name": "user3"}).Find(context.Background(), &users6)
	if len(users1)-len(users6) != int(name3Count) {
		t.Errorf("Should find all users's name not equal 3")
	}

	DB.Not(map[string]interface{}{"name": "user3", "company_id": nil}).Find(context.Background(), &users7)
	...
	DB.Not("name", []string{"user3"}).Find(context.Background(), &users8)
	if len(users1)-len(users8) != int(name3Count) {
		t.Errorf("Should find all users's name not equal 3")
	}

	var name2Count int64
	DB.Table("users").Where("name = ?", "user2").Count(context.Background(), &name2Count)
	DB.Not("name", []string{"user3", "user2"}).Find(context.Background(), &users9)
...
func TestFillSmallerStruct(t *testing.T) {
	...
	DB.Table("users").Where("name = ?", user1.Name).First(context.Background(), &simpleUser)
...
func TestFindOrInitialize(t *testing.T) {
	...
	DB.Where(&User{Name: "find or init", Age: 33}).FirstOrInit(context.Background(), &user1)
	if user1.Name != "find or init" || user1.Id != 0 || user1.Age != 33 {
		t.Errorf("user should be initialized with search value")
	}

	DB.Where(User{Name: "find or init", Age: 33}).FirstOrInit(context.Background(), &user2)
	if user2.Name != "find or init" || user2.Id != 0 || user2.Age != 33 {
		t.Errorf("user should be initialized with search value")
	}

	DB.FirstOrInit(context.Background(), &user3, map[string]interface{}{"name": "find or init 2"})
	if user3.Name != "find or init 2" || user3.Id != 0 {
		t.Errorf("user should be initialized with inline search value")
	}

	DB.Where(&User{Name: "find or init"}).Attrs(User{Age: 44}).FirstOrInit(context.Background(), &user4)
	if user4.Name != "find or init" || user4.Id != 0 || user4.Age != 44 {
		t.Errorf("user should be initialized with search value and attrs")
	}

	DB.Where(&User{Name: "find or init"}).Assign("age", 44).FirstOrInit(context.Background(), &user4)
	if user4.Name != "find or init" || user4.Id != 0 || user4.Age != 44 {
		t.Errorf("user should be initialized with search value and assign attrs")
	}

	DB.Save(context.Background(), &User{Name: "find or init", Age: 33})
	DB.Where(&User{Name: "find or init"}).Attrs("age", 44).FirstOrInit(context.Background(), &user5)
	if user5.Name != "find or init" || user5.Id == 0 || user5.Age != 33 {
		t.Errorf("user should be found and not initialized by Attrs")
	}

	DB.Where(&User{Name: "find or init", Age: 33}).FirstOrInit(context.Background(), &user6)
	if user6.Name != "find or init" || user6.Id == 0 || user6.Age != 33 {
		t.Errorf("user should be found with FirstOrInit")
	}

	DB.Where(&User{Name: "find or init"}).Assign(User{Age: 44}).FirstOrInit(context.Background(), &user6)
...
func TestFindOrCreate(t *testing.T) {
	...
	DB.Where(&User{Name: "find or create", Age: 33}).FirstOrCreate(context.Background(), &user1)
	if user1.Name != "find or create" || user1.Id == 0 || user1.Age != 33 {
		t.Errorf("user should be created with search value")
	}

	DB.Where(&User{Name: "find or create", Age: 33}).FirstOrCreate(context.Background(), &user2)
	if user1.Id != user2.Id || user2.Name != "find or create" || user2.Id == 0 || user2.Age != 33 {
		t.Errorf("user should be created with search value")
	}

	DB.FirstOrCreate(context.Background(), &user3, map[string]interface{}{"name": "find or create 2"})
	if user3.Name != "find or create 2" || user3.Id == 0 {
		t.Errorf("user should be created with inline search value")
	}

	DB.Where(&User{Name: "find or create 3"}).Attrs("age", 44).FirstOrCreate(context.Background(), &user4)
	...
	DB.Where(&User{Name: "find or create 3"}).Assign("age", 55).FirstOrCreate(context.Background(), &user4)
	...
	DB.Where(&User{Name: "find or create 4"}).Assign(User{Age: 44}).FirstOrCreate(context.Background(), &user4)
	...
	DB.Where(&User{Name: "find or create"}).Attrs("age", 44).FirstOrInit(context.Background(), &user5)
	if user5.Name != "find or create" || user5.Id == 0 || user5.Age != 33 {
		t.Errorf("user should be found and not initialized by Attrs")
	}

	DB.Where(&User{Name: "find or create"}).Assign(User{Age: 44}).FirstOrCreate(context.Background(), &user6)
	if user6.Name != "find or create" || user6.Id == 0 || user6.Age != 44 {
		t.Errorf("user should be found and updated with assigned attrs")
	}

	DB.Where(&User{Name: "find or create"}).Find(context.Background(), &user7)
	if user7.Name != "find or create" || user7.Id == 0 || user7.Age != 44 {
		t.Errorf("user should be found and updated with assigned attrs")
	}

	DB.Where(&User{Name: "find or create embedded struct"}).Assign(User{Age: 44, CreditCard: CreditCard{Number: "1231231231"}, Emails: []Email{{Email: "jinzhu@assign_embedded_struct.com"}, {Email: "jinzhu-2@assign_embedded_struct.com"}}}).FirstOrCreate(context.Background(), &user8)
	if DB.Where("email = ?", "jinzhu-2@assign_embedded_struct.com").First(context.Background(), &Email{}).RecordNotFound() {
		t.Errorf("embedded struct email should be saved")
	}

	if DB.Where("email = ?", "1231231231").First(context.Background(), &CreditCard{}).RecordNotFound() {
...
func TestSelectWithEscapedFieldName(t *testing.T) {
	...
	var names []string
	DB.Model(User{}).Where(&User{Name: "EscapedFieldNameUser"}).Pluck(context.Background(), "\"name\"", &names)
...
func TestSelectWithVariables(t *testing.T) {
	DB.Save(context.Background(), &User{Name: "jinzhu"})

	rows, _ := DB.Table("users").Select("? as fake", gorm.Expr("name")).Rows(context.Background())
...
func TestSelectWithArrayInput(t *testing.T) {
	DB.Save(context.Background(), &User{Name: "jinzhu", Age: 42})

	var user User
	DB.Select([]string{"name", "age"}).Where("age = 42 AND name = 'jinzhu'").First(context.Background(), &user)
...
func TestPluckWithSelect(t *testing.T) {
	...
	var userAges []string
	err := DB.Model(&User{}).Where("age = ?", 25).Select(selectStr).Pluck(context.Background(), "user_age", &userAges).Error
	if err != nil {
		t.Error(err)
	}
