...
func TestPrefixColumnNameForEmbeddedStruct(t *testing.T) {
	dialect := DB.NewScope(context.Background(), &EngadgetPost{}).Dialect()
	engadgetPostScope := DB.NewScope(context.Background(), &EngadgetPost{})
	...
	hnScope := DB.NewScope(context.Background(), &HNPost{})
...
func TestSaveAndQueryEmbeddedStruct(t *testing.T) {
	...
	if DB.NewScope(context.Background(), &HNPost{}).PrimaryField() == nil {
		t.Errorf("primary key with embedded struct should works")
	}

	for _, field := range DB.NewScope(context.Background(), &HNPost{}).Fields() {
