...
func TestBelongsToOverrideForeignKey1(t *testing.T) {
	...
	if relation, ok := DB.NewScope(context.Background(), &User{}).FieldByName("Profile"); ok {
...
func TestForeignKey(t *testing.T) {
	for _, structField := range DB.NewScope(context.Background(), &User{}).GetStructFields() {
	...
	for _, structField := range DB.NewScope(context.Background(), &Email{}).GetStructFields() {
	...
	for _, structField := range DB.NewScope(context.Background(), &Post{}).GetStructFields() {
	...
	for _, structField := range DB.NewScope(context.Background(), &Comment{}).GetStructFields() {
...
func testForeignKey(t *testing.T, source interface{}, sourceFieldName string, target interface{}, targetFieldName string) {
	...
	targetScope := DB.NewScope(context.Background(), target)
	targetTableName := targetScope.TableName()
	modelScope := DB.NewScope(context.Background(), source)
